...
class SunmiAidlPrint {
  ...
  // PRINTER INITIAL
  static Future<Null> initPrinter() async {
    await _channel.invokeMethod("initPrinter");
  }

  static Future<Null> bindPrinter() async {
    await _channel.invokeMethod("bindPrinter");
  }

  static Future<Null> unbindPrinter() async {
    await _channel.invokeMethod("unbindPrinter");
  }
  ...
  static Future<Null> printTextWithFont(
      {String? text, String? fontName, int? fontSize}) async {
    Map<String, dynamic> args = <String, dynamic>{};
    args.putIfAbsent("text", () => text);
    args.putIfAbsent("fontName", () => fontName);
    args.putIfAbsent("fontSize", () => fontSize);
  ...
  static Future<Null> printColumnText(
      {List<String>? text, List<int>? width, List<int>? align}) async {
    Map<String, dynamic> args = <String, dynamic>{};
    args.putIfAbsent("text", () => text);
  ...
  static Future<Null> printQRCode(
      {String? text, int? moduleSize, ERRORLEVEL? errorLevel}) async {
    Map<String, dynamic> args = <String, dynamic>{};
    args.putIfAbsent("text", () => text);
    args.putIfAbsent(
        "moduleSize",
  ...
  static Future<Null> printBarcode(
      {String? text,
      SYMBOLOGY? symbology,
      int? height,
      int? width,
      TEXTPOS? textPosition}) async {
    Map<String, dynamic> args = <String, dynamic>{};
    ...
    args.putIfAbsent(
        "textPosition",
        () => textPosition == TEXTPOS.NO_PRINT_TEXT
            ? 0
            : textPosition == TEXTPOS.ABOVE_BARCODE
                ? 1
                : textPosition == TEXTPOS.BELOW_BARCODE
                    ? 2
                    : textPosition == TEXTPOS.BOTH
                        ? 3
                        : 0);
